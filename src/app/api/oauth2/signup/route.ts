...
async function processPost(request: NextRequest) {
  ...
  const registrationFlow = !flowId
    ...
    : await frontendApi.getRegistrationFlow({ id: flowId });

  const csrf_token = getCsrfToken(registrationFlow) ?? "";

  let messages: any[] | undefined;

  try {
    if (traits && password && transient_payload) {
  ...
  return NextResponse.json(
    { flow_id: registrationFlow.id, session, error: messages },
    { status: 200 },
  );
}
